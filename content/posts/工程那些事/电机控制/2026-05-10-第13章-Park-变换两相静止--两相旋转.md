...
dq 坐标系随转子旋转，`θ_el` 是 dq 坐标系相对于 αβ 坐标系的角度。
...
```mermaid
flowchart TD
    A[三相电流采样<br/>i_a, i_b, i_c] -->|Clarke 变换| B[两相静止<br/>i_alpha, i_beta]
    B -->|Park 变换<br/>theta_el 来自位置传感器| C[两相旋转<br/>i_d, i_q]
    C -->|PI 控制| D[两相旋转电压<br/>u_d, u_q]
    D -->|逆 Park 变换| E[两相静止电压<br/>u_alpha, u_beta]
    E -->|逆 Clarke 变换| F[三相电压调制<br/>u_a, u_b, u_c]
    F -->|SVPWM| G[PWM 输出]
```

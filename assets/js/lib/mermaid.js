...
function normalizeKeys(value, keyMap) {
...
function normalizeFlowchartConfig(value) {
  if (!value || typeof value !== 'object') return value
  return {
    ...value,
    htmlLabels: value.htmllabels ?? value.htmlLabels,
    nodeSpacing: value.nodespacing ?? value.nodeSpacing,
    rankSpacing: value.rankspacing ?? value.rankSpacing
  }
}
...
function getThemeVariables(darkMode) {
  const variables = mermaidConfig.themevariables
  if (!variables) return undefined
  return normalizeKeys(darkMode ? variables.dark : variables.light, themeVariableKeyMap)
}
...
async function loadMermaid({ theme, darkMode, selector }) {
  ...
  mermaid.initialize({
    ...
    flowchart: normalizeFlowchartConfig(mermaidConfig.flowchart),
